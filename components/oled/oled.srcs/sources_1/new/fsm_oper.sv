...
spi SPI_COMP(.clk(clk), .rst(rst), .en(spi_en),
	.indata(spi_data), .sdo(sdo), .sclk(sclk), .fin(spi_fin));
	
assign spi_data = dc?spi_data_data:spi_data_cmd;
assign spi_en = dc?spi_en_data:up_spi_en;

delay #(.nbits(12)) DELAY_COMP(
    .clk(clk), .rst(rst), .del_ms(delay_ms),
...
//font templates
rom CHAR_LIB_COM(.clk(clk), .en(romen), .addr(addr), .dout(romout));
